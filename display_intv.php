...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $whatYouDo = mysqli_real_escape_string($conn, $_POST['what_you_do']);
    $existingDisease = mysqli_real_escape_string($conn, $_POST['what_is_your_existing_desease']);
    $familyHistory = mysqli_real_escape_string($conn, $_POST['have_you_a_family_history_desease']);
    $allergy = mysqli_real_escape_string($conn, $_POST['have_you_a_allergy']);
    $year = date("Y");

    // Insert form data into the database
    $sql = "INSERT INTO intv (IDNumber, what_you_do, what_is_your_existing_desease, 
                              have_you_a_family_history_desease, have_you_a_allergy, years) 
            VALUES ('$idNumber', '$whatYouDo', '$existingDisease', '$familyHistory', '$allergy', '$year')";
    ...
    if (mysqli_query($conn, $sql)) {
        ...
        $_SESSION['intv_data'] = [
            ...
            'have_you_a_allergy' => $allergy,
            'years' => $year,
        ];

        // Redirect to the display page after a successful insert
        header("Location: display_intv_information.php");
        exit();
    } else {
        // Error handling
...
$selectQuery = "SELECT Num FROM intv WHERE years = ? ORDER BY Num ASC";
...
while ($row = $result->fetch_assoc()) {
    ...
    // Update Num in intv for the selected year
    $updateIntv = "UPDATE intv SET Num = $new_num WHERE Num = $old_num AND years = $selectedYear";
    mysqli_query($conn, $updateIntv);
...
// Query to fetch records from 'intv' for display
$sql = "SELECT Num, IDNumber, what_you_do, what_is_your_existing_desease, 
               have_you_a_family_history_desease, have_you_a_allergy, years 
        FROM intv 
        WHERE years = ?
        ORDER BY Num ASC";
    ...
    <div class="main-content">
        ...
        <div id="searchContainer">
            <select id="columnSelect">
                ...
                <option value="3">Existing Disease</option>
                <option value="4">Family History of Disease</option>
        ...
        <h3>Student Consultation Records for School Year <?php echo $selectedYear; ?></h3>
        


        <?php if ($resultIntv->num_rows > 0): ?>
        <table id="recordsTable">
            <thead>
                <tr>
                    <th data-order="asc">#</th>
                    <th data-order="asc">ID Number</th> <!-- New header for ID Number -->
                    <th data-order="asc">Activity</th>
                    <th data-order="asc">Existing Disease</th>
                    <th data-order="asc">Family History of Disease</th>
                    <th data-order="asc">Allergies</th>
                    <th>Action</th>
                </tr>
            </thead>
            <tbody>
                <?php while ($row = $resultIntv->fetch_assoc()): ?>
                    <tr>
                        <td><?php echo $row['Num']; ?></td>
                        <td><?php echo $row['IDNumber']; ?></td> <!-- Display ID Number -->
                        <td><?php echo $row['what_you_do']; ?></td>
                        <td><?php echo $row['what_is_your_existing_desease']; ?></td>
                        <td><?php echo $row['have_you_a_family_history_desease']; ?></td>
                        <td><?php echo $row['have_you_a_allergy']; ?></td>
                        <td>
                            <button class="btn1" onclick="openEditModal(<?php echo htmlspecialchars(json_encode($row)); ?>)">Edit</button>
                            <button class="btn" onclick="deleteRecord(<?php echo $row['Num']; ?>)">Delete</button>
                        </td>
                    </tr>
                <?php endwhile; ?>
            </tbody>
        ...
        <?php else: ?>
            <p class="no-records">No consultation records found.</p>
        <?php endif; ?>
    </div>
</div>
...
<script>
   function printTable() {
    ...
    var headerText = `<h1>Student Consultation Records</h1>`;
    ...
    printWindow.document.write(`
        <html>
        <head>
            <title>Student Consultation Records</title>
            <style>
                body {
...
<div id="editModal" class="modal" style="display:none;">
    <div class="modal-content">
        ...
        <form id="editForm">
            ...
            <label for="what_you_do">Activity:</label>
            <input type="text" name="what_you_do" id="editWhatYouDo" required>
            <label for="existing_disease">Existing Disease:</label>
            <input type="text" name="existing_disease" id="editExistingDisease" required>
            <label for="family_history">Family History of Disease:</label>
            <input type="text" name="family_history" id="editFamilyHistory" required>
            <label for="allergies">Allergies:</label>
            <input type="text" name="allergies" id="editAllergies" required>
...
function openEditModal(rowData) {
    ...
    document.getElementById('editWhatYouDo').value = rowData.what_you_do;
    document.getElementById('editExistingDisease').value = rowData.what_is_your_existing_desease;
    document.getElementById('editFamilyHistory').value = rowData.have_you_a_family_history_desease;
    document.getElementById('editAllergies').value = rowData.have_you_a_allergy;
    ...
    <script>
        ...
        function deleteRecord(id) {
            if (confirm("Are you sure you want to delete this record?")) {
                window.location.href = 'delete_intv.php?id=' + id; // Redirect to delete script
            }
        }
        ...
        const selectedColumn = columnSelect.value; // Get selected column index
