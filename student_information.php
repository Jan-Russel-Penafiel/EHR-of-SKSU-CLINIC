...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $year = mysqli_real_escape_string($conn, $_POST['Yr']);
    $section = mysqli_real_escape_string($conn, $_POST['Section']);

    // Calculate age based on birthdate
    $age = date_diff(date_create($birthdate), date_create('now'))->y;

    // Insert form data into the database (Num will be auto-incremented)
    $sql = "INSERT INTO personal_info (FirstName, LastName, IDNumber, GmailAccount, Birthdate, Age, Gender, Course, Yr, Section) 
            VALUES ('$firstName', '$lastName', '$idNumber', '$gmailAccount', '$birthdate', '$age', '$gender', '$course', '$year', '$section')";
...
$selectQuery = "SELECT Num FROM personal_info WHERE YEAR(years) = ? ORDER BY Num ASC";
...
while ($row = $result->fetch_assoc()) {
    ...
    // Update Num in personal_info for the selected year
    $updatePersonalInfo = "UPDATE personal_info SET Num = $new_num WHERE Num = $old_num AND YEAR(years) = $selectedYear";
    mysqli_query($conn, $updatePersonalInfo);

    // Update Num in medical_history for the selected year
    $updateMedicalHistory = "UPDATE medical_history SET Num = $new_num WHERE Num = $old_num";
...
// Query to fetch records from 'personal_info' for display
$sql = "SELECT Num, FirstName, LastName, IDNumber, GmailAccount, Birthdate, Age, Gender, Course, Yr, Section, YEAR(CURDATE()) - YEAR(Birthdate) AS Age
        FROM personal_info 
        WHERE YEAR(years) = ?
        ORDER BY Num ASC";
...
<head>
    ...
    <title>School Clinic DBMS</title>
...
<div class="container">
    <div class="main-content">
        ...
        <h3>Registered Students Records for School Year <?php echo $selectedYear; ?></h3>

<?php if (mysqli_num_rows($resultIntv) > 0): ?>
    <table id="recordsTable">
        <thead>
            <tr>
                ...
                <th data-order="asc">Birth Date</th>
                <th data-order="asc">Age</th>
                <th data-order="asc">Gender</th>
        ...
        <tbody>
            <?php while ($row = mysqli_fetch_assoc($resultIntv)): ?>
                <tr>
                    <td><?php echo htmlspecialchars($row['Num']); ?></td>
                    <td><?php echo htmlspecialchars($row['IDNumber']); ?></td> <!-- Display ID Number -->
                    ...
                    <td><?php echo htmlspecialchars($row['Yr']); ?></td>
...
<?php else: ?>
    <p class="no-records">No registered student records found for the selected year.</p>
<?php endif; ?>
<script>
   function printTable() {
    ...
    var headerText = `<h1>Registered Students Records</h1>`;
...
<div id="editModal" class="modal" style="display:none;">
    <div class="modal-content">
        ...
        <form id="editForm">
            ...
            <!-- Add IDNumber input field -->
            <label for="idNumber">ID Number:</label>
            <input type="text" name="IDNumber" id="editIDNumber" required readonly>
            ...
            <input type="text" name="gmail" id="editGmail" required readonly>
...
function openEditModal(rowData) {
    ...
    document.getElementById('editIDNumber').value = rowData.IDNumber; // Add this line to set the IDNumber
    document.getElementById('editFirstName').value = rowData.FirstName;
    document.getElementById('editLastName').value = rowData.LastName;
    document.getElementById('editGmail').value = rowData.GmailAccount;

    document.getElementById('editModal').style.display = 'block'; // Make the modal visible
}
...
.modal-content {
    ...
    margin: 10% auto; /* Increased margin for positioning */
...
button {
    ...
    margin-top: 5px; /* Increased margin on top */
}
...
// Toggle dropdown menu visibility
      ...
      function deleteRecord(id) {
        if (confirm('Are you sure you want to delete this record?')) {
            // Redirect to a delete endpoint or use AJAX to delete
            window.location.href = 'delete_student_information.php?id=' + id; // Assuming you have a delete script
        }
    }
