...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $sql = "INSERT INTO illmed (IllName, MedName, Prescription, Temperature, BloodPressure, Status, Appointment_Date, years, IDNumber) 
            VALUES ('$illName', '$medName', '$prescription', '$temperature', '$bloodPressure', '$status', '$appointmentDate', '$year', '$idNumber')";

    if (mysqli_query($conn, $sql)) {
        // Save the submitted data to session
        $_SESSION['illmed_data'] = [
            'IllName' => $illName,
            'MedName' => $medName,
        ...
        header("Location: display_illmed_information.php");
        exit();
    } else {
        // Error handling
...
$selectQuery = "SELECT Num FROM illmed WHERE years = ? ORDER BY Appointment_Date ASC";
...
while ($row = $result->fetch_assoc()) {
    ...
    // Update Num in illmed for the selected year
    $updateIllmed = "UPDATE illmed SET Num = $new_num WHERE Num = $old_num AND years = $selectedYear";
    mysqli_query($conn, $updateIllmed);
...
// Query to fetch records from 'illmed' for display
$sql = "SELECT Num, IllName, MedName, Prescription, Temperature, BloodPressure, Status, Appointment_Date, years, IDNumber
        FROM illmed 
        WHERE years = ?
        ORDER BY Num ASC";
...
select:focus {
    ...
    outline: none; /* Remove default outline */
    box-shadow: 0 0 5px rgba(102, 175, 233, .5); /* Add a subtle shadow */
}
    ...
    <div class="main-content">
        ...
        <div id="searchContainer">
            <select id="columnSelect">
                ...
                <option value="2">Illness Name</option>
                <option value="3">Medication Name</option>
              
                <option value="4">Prescription</option>
                <option value="5">Temperature</option>
                <option value="6">Blood Pressure</option>
                <option value="7">Status</option>
                <option value="8">Appointment Date</option>
            </select>
        ...
        <h3>Student Treatment Records for Year <?php echo $selectedYear; ?></h3>
    ...
    <table id="recordsTable">
        <thead>
            <tr>
                ...
                <th data-order="asc">Illness Name</th>
                <th data-order="asc">Medication Name</th>
            
                <th data-order="asc">Prescription</th>
                <th data-order="asc">Temperature</th>
                <th data-order="asc">Blood Pressure</th>
        ...
        <tbody>
            <?php while ($row = mysqli_fetch_assoc($resultIntv)): ?>
                <tr>
                    ...
                    <td><?= date("F j, Y, h:i A", strtotime($row['Appointment_Date'])) ?></td>
                    <td>
...
<?php else: ?>
    <p class="no-records">No treatment records found for the selected year.</p>
<?php endif; ?>
...
<script>
   function printTable() {
    ...
    var headerText = `<h1>Student Treatment Records</h1>`;
    ...
    printWindow.document.write(`
        <html>
        <head>
            <title>Student Treatment Records</title>
            <style>
                body {
...
<select name="IllName" id="editIllName" required>
...
<select name="MedName" id="editMedName" required>
<option value="Paracetamol">1. Paracetamol</option>
        ...
        <!-- Add Prescription Field in the Edit Modal -->
<div class="form-group">
    <label for="Prescription">Prescription:</label>
    <input type="text" name="Prescription" id="editPrescription" required>
</div>
...
function openEditModal(rowData) {
    ...
    document.getElementById('editIllName').value = rowData.IllName;
    document.getElementById('editMedName').value = rowData.MedName;
    document.getElementById('editPrescription').value = rowData.Prescription;
    ...
    document.getElementById('editAppointmentDate').value = new Date(rowData.Appointment_Date).toISOString().slice(0, -1);
    ...
    <script>
        ...
        function deleteRecord(id) {
            if (confirm("Are you sure you want to delete this record?")) {
                window.location.href = 'delete_illmed.php?id=' + id; // Redirect to delete script
            }
        }
